...
export default function Header() {
    ...
    const handleScroll = () => {
        ...
        if (offset > 100) { // Adjust this value based on when you want the header to become sticky
            setIsSticky(true);
        } else {
            setIsSticky(false);
        }
    ...
    return (
        <>
            <div className='relative w-full'>
                ...
                <div
                    className={`flex w-full h-14 items-center z-10 justify-between px-7 transition-all duration-300 ${isSticky ? 'fixed top-0 left-0 right-0 ' : 'absolute -bottom-7'
                        }`}
